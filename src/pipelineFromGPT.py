...
from grid_llava import main as run_grid_llava
from grid_optical_flow import main as run_grid_optical_flow
from llava import main as run_llava
...
def _thresholds_for_detector(threshold_sweep: bool) -> List[float]:
    """Thresholds used for detector calibration sweeps."""
    if not threshold_sweep:
        return [0.0]
    # Historically used range: [-0.5, 2.0] step 0.1 seconds.
    return [0.1 * i for i in range(-5, 21)]
...
@click.option(
    ...
    "-m",
    type=click.Choice(
        ["mlnet", "tasednet", "transalnet", "optical-flow", "llava", "chatgpt"],
...
@click.option(
    "--threshold-sweep/--no-threshold-sweep",
    default=True,
    help="Run detector across a threshold range for calibration.",
)
...
def main(
    ...
    if apply_vlm:
        ...
        click.echo("-" * 40)
        run_llava(data_path, output_path, config_path, method)

    if method in {"llava", "chatgpt"}:
        click.echo("-" * 40)
        click.echo(f"Processing {method} responses into a grid...")
        click.echo("-" * 40)
        run_grid_llava(data_path, output_path, config_path, flicker_handling)
    ...
    # -------------------------
    use_attention = method in {"mlnet", "tasednet", "transalnet", "llava", "chatgpt"}

    for threshold in _thresholds_for_detector(threshold_sweep):
        click.echo("-" * 40)
        click.echo(f"Running detector (threshold={threshold:+.1f}s)...")
        click.echo("-" * 40)
        run_detector_morton(output_path, config_path, use_attention, threshold)

        if annotations_path:
            click.echo("-" * 40)
            click.echo("Running evaluation...")
            click.echo("-" * 40)
            run_evaluate(output_path, annotations_path, threshold)
