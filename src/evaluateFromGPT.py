...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Threshold value used for the detector run (for logging/calibration).",
    )
...
def main(data_path: str, ground_truth_path: str, threshold: float):
    ...
    print("=================")
    print(f"threshold={threshold:+.1f}s | TP={TP} FP={FP} FN={FN} TN={TN}")
    print(f"F1={f1_score} | sensitivity={sensitivity} | specificity={specificity} | mean_IoU={mean_iou}")

    # Keep writing a compact calibration row for threshold sweeps.
    write_csvline(
        [threshold, TP, FP, FN, TN, f1_score, sensitivity, specificity, mean_iou],
        filename="threshold_sweep",
        path="thresholds/",
    )
...
if __name__ == "__main__":
    ...
    main(args.data_path, args.ground_truth_path, args.threshold)
