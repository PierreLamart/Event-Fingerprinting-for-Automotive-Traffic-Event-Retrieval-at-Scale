...
def save_cell_value_csv(cell_value_map, output_path, config):
    ...
    csv_path = output_path + "/cell_values.csv"
...
def calculate_grid_cell_positions(image, config):
    """Calculate pixel bounding boxes for every configured cell.

    The config expresses each grid as proportional coordinates in [0, 1].
    This converts those proportions to pixel coordinates for a given image.

    Supports both:
      * OpenCV / numpy images (``image.shape``)
      * PIL images (``image.size``)
    """
    ...
    if hasattr(image, "shape"):
        ...
        raise TypeError("Unsupported image type for grid cell calculation")
    ...
    for grid_config in config["grids"]:
        ...
        # Concatenate all grid cells into a single global cell list.
