"""Event-window detector based on sequences of Morton codes.

The pipeline encodes per-frame grid activity into a single Morton code.
This detector searches for ordered transitions between *cell ranges* that
match the expected scenario (left-to-right or right-to-left crossing).

High level:
1) map Morton codes -> cell ids (range lookup)
2) build candidate sequences that move monotonically across cells
3) validate candidates using timing and length constraints
4) pick the candidate with the most cell visits and output its window
"""
...
@dataclass
class Sequence:
    """A candidate ordered sequence through the grid.

    Attributes
    ----------
    path:
        List of (cell_id, frame_id) tuples in the order they were observed.
    direction:
        "left" means cell id increasing over time; "right" means decreasing.
    interrupts:
        How many non-monotonic steps have been tolerated so far.
    active:
        Whether the sequence is still accepting new nodes.
    """
...
def frames_to_seconds(nr_frames: int, fps: int) -> float:
    return nr_frames / fps


def get_matching_cell_key(morton_code: float, cell_morton: Dict[int, Tuple[float, float]]) -> Optional[int]:
...
def is_valid_sequence(
    sequence_path: List[Tuple[int, int]],
...
def get_ordered_sequences(
    ...
    fps: int,
    tolerance: int = 1,
    same_cell_restart_seconds: float = 3.0,
):
    """Split a path into monotonic sequences.

    The raw path is a time-ordered list of nodes (cell_id, frame_id).
    This function groups it into candidate sequences that move consistently
    across cells in the specified direction.
    """
    ...
    for node in path:
        ...
        for active_seq in active_sequences_copy:
            ...
            if node[0] > prev[0] and direction == "left":
                ...
                # If we stay in the same cell but the time gap is very large,
                # close the current sequence and start a new one.
                if frames_to_seconds(frame_diff, fps) > same_cell_restart_seconds:
...
def get_sequence(
    ...
    """Return the best valid sequence or ``None``."""
    path = []
    ...
    for direction in ("left", "right"):
        ordered_sequences = get_ordered_sequences(path, direction, fps=fps)
        #print("-----------------------")
        #print(direction)
        #print(path)
...
def main(
    ...
    # Copy because we may tweak the values for calibration sweeps.
    cell_gap_time_limits = [list(x) for x in detector_config["cell_gap_time_limits"]]
    event_length_limit = detector_config["event_length_limit_seconds"]
    # The detector uses min/max allowed time gaps between cell transitions.
    for i in range(len(cell_gap_time_limits)):
        cell_gap_time_limits[i][1] = round(cell_gap_time_limits[i][1], 3)
